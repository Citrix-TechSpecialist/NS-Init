...
class capolicy(base_resource) :
	...
	@property
	def name(self) :
		ur"""Name for the content accelerator policy. Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters. Can be changed after the policy is created.
		"""
		try :
			return self._name
	...
	@property
	def rule(self) :
		ur"""Expression that determines which requests or responses match the content accelerator policy. When specifying the rule in the CLI, the description must be enclosed within double quotes.
		"""
		try :
			return self._rule
		except Exception as e:
	...
	@property
	def action(self) :
		ur"""Name of content accelerator action to be executed when the rule is evaluated to true.
		"""
		try :
			return self._action
		except Exception as e:
	...
	@property
	def comment(self) :
		ur"""Information about the content accelerator policy.
		"""
		try :
	...
	@property
	def isdefault(self) :
		ur"""A value of true is returned if it is a default content accelerator policy.
		"""
		try :
