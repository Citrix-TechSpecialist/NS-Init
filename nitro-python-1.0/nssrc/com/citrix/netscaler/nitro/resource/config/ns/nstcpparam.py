...
class nstcpparam(base_resource) :
	...
	def mptcpclosemptcpsessiononlastsfclose(self) :
		ur"""Allow to send DATA FIN or FAST CLOSE on mptcp connection while sending FIN or RST on the last subflow.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
	...
	@property
	def builtin(self) :
		ur"""Flag to determine if the tcp param is built-in or not.<br/>Possible values = MODIFIABLE, DELETABLE, IMMUTABLE, PARTITION_ALL.
		"""
		try :
