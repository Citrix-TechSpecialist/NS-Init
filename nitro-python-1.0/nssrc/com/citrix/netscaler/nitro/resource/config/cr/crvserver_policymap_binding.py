...
class crvserver_policymap_binding(base_resource) :
	...
	@property
	def priority(self) :
		ur"""An unsigned integer that determines the priority of the policy relative to other policies bound to this cache redirection virtual server. The lower the value, higher the priority. Note: This option is available only when binding content switching, filtering, and compression policies to a cache redirection virtual server.
		"""
		try :
	...
	@property
	def bindpoint(self) :
		ur"""For a rewrite policy, the bind point to which to bind the policy. Note: This parameter applies only to rewrite
	...
	@property
	def invoke(self) :
		ur"""Invoke a policy label if this policy's rule evaluates to TRUE (valid only for default-syntax policies such as
	...
	def get(cls, service, name) :
		ur""" Use this API to fetch crvserver_policymap_binding resources.
		"""
		try :
	...
	def get_filtered(cls, service, name, filter_) :
		ur""" Use this API to fetch filtered set of crvserver_policymap_binding resources.
	...
	def count(cls, service, name) :
		ur""" Use this API to count crvserver_policymap_binding resources configued on NetScaler.
		"""
		try :
			obj = crvserver_policymap_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		ur""" Use this API to count the filtered set of crvserver_policymap_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = crvserver_policymap_binding()
