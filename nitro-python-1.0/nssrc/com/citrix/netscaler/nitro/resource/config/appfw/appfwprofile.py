...
class appfwprofile(base_resource) :
	...
	def __init__(self) :
		...
		self._dosecurecreditcardlogging = ""
		self._streaming = ""
		self._trace = ""
	...
	def inspectcontenttypes(self) :
		ur"""One or more InspectContentType lists. 
	...
	def creditcardmaxallowed(self) :
		ur"""Maximum number of credit card numbers that can appear on a web page served by your protected web sites. Pages that contain more credit card numbers are blocked if 'block' action is enabled. It doesn't apply to x-out.<br/>Maximum length =  255.
		"""
		try :
	...
	def dosecurecreditcardlogging(self) :
		ur"""Setting this option logs credit card numbers in the response when the match is found.<br/>Default value: ON<br/>Possible values = ON, OFF.
		"""
		try :
	...
	@property
	def trace(self) :
		ur"""Toggle  the state of trace.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
			return self._trace
		except Exception as e:
	...
	def stripxmlcomments(self) :
		ur"""Strip XML comments before forwarding a web page sent by a protected web site in response to a user request.<br/>Default value: none<br/>Possible values = none, all.
		"""
		try :
	...
	def exemptclosureurlsfromsecuritychecks(self) :
		ur"""Exempt URLs that pass the Start URL closure check from SQL injection, cross-site script, field format and field consistency security checks at locations other than headers.<br/>Default value: ON<br/>Possible values = ON, OFF.
		"""
		try :
