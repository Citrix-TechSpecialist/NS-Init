...
class appfwprofile_xmlxss_binding(base_resource) :
	...
	def as_scan_location_xmlxss(self) :
		ur"""Location of XSS injection exception - XML Element or Attribute.<br/>Possible values = ELEMENT, ATTRIBUTE.
		"""
		try :
	...
	def isregex_xmlxss(self) :
		ur"""Is the XML XSS exempted field name a regular expression?.<br/>Possible values = REGEX, NOTREGEX.
		"""
		try :
	...
	@property
	def xmlxss(self) :
		ur"""Exempt the specified URL from the XML cross-site scripting (XSS) check.
	...
	def get(cls, service, name) :
		ur""" Use this API to fetch appfwprofile_xmlxss_binding resources.
		"""
		try :
	...
	def get_filtered(cls, service, name, filter_) :
		ur""" Use this API to fetch filtered set of appfwprofile_xmlxss_binding resources.
	...
	def count(cls, service, name) :
		ur""" Use this API to count appfwprofile_xmlxss_binding resources configued on NetScaler.
		"""
		try :
			obj = appfwprofile_xmlxss_binding()
			obj.name = name
	...
	def count_filtered(cls, service, name, filter_) :
		ur""" Use this API to count the filtered set of appfwprofile_xmlxss_binding resources.
		Filter string should be in JSON format.eg: "port:80,servicetype:HTTP".
		"""
		try :
			obj = appfwprofile_xmlxss_binding()
