...
class caaction(base_resource) :
	...
	@property
	def name(self) :
		ur"""Name of the content accelerator action. Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters.
		"""
		try :
			return self._name
	...
	@property
	def comment(self) :
		ur"""Information about the content accelerator action.
		"""
		try :
	...
	@property
	def newname(self) :
		ur"""New name for the content accelerator action. 
	...
	@property
	def builtin(self) :
		ur"""Flag to determine whether content accelerator action is built-in or not.<br/>Possible values = MODIFIABLE, DELETABLE, IMMUTABLE, PARTITION_ALL.
		"""
		try :
