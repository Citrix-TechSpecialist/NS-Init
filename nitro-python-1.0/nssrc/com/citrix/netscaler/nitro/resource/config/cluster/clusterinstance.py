...
class clusterinstance(base_resource) :
	...
	def __init__(self) :
		...
		self._nodegroupstatewarning = False
		self._licensemismatch = False
		self._jumbonotsupported = False
		self._clusternoheartbeatonnode = False
		self._clusternolinksetmbf = False
		self._clusternospottedip = False
	...
	def deadinterval(self) :
		ur"""Amount of time, in seconds, after which nodes that do not respond to the heartbeats are assumed to be down.If the value is less than 3 sec, set the helloInterval parameter to 200 msec.<br/>Default value: 3<br/>Minimum length =  1<br/>Maximum length =  60.
		"""
		try :
	...
	def hellointerval(self) :
		ur"""Interval, in milliseconds, at which heartbeats are sent to each cluster node to check the health status.Set the value to 200 msec, if the deadInterval parameter is less than 3 sec.<br/>Default value: 200<br/>Minimum length =  200<br/>Maximum length =  1000.
		"""
		try :
	...
	@property
	def inc(self) :
		ur"""This option is required if the cluster nodes reside on different networks.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
			return self._inc
		except Exception as e:
	...
	def processlocal(self) :
		ur"""By turning on this option packets destined to a service in a cluster will not under go any steering.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
	...
	@property
	def nodegroup(self) :
		ur"""The node group in a Cluster system used for transition from L2 to L3.
		"""
		try :
	...
	def nodegroupstatewarning(self) :
		ur"""This argumnt is used to determind whether all the cluster nodes are bound to nodegroup with state set.
		"""
		try :
	...
	def clusternoheartbeatonnode(self) :
		ur"""HB is not seen on the backplane interface of member node.
		"""
		try :
	...
	def clusternolinksetmbf(self) :
		ur"""MBF is enabled but linkset is not configured .
		"""
		try :
	...
	def clusternospottedip(self) :
		ur"""There is no spotted SNIP or MIP.
		"""
		try :
