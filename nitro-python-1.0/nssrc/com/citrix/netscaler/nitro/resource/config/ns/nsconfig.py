...
class nsconfig(base_resource) :
	...
	def weakpassword(self) :
		ur"""Option to list all weak passwords (not adhering to strong password requirements). Takes config file as input, if no input specified, running configuration is considered. Command => query ns config -weakpassword  / query ns config -weakpassword /nsconfig/ns.conf.
		"""
		try :
	...
	@property
	def config(self) :
		ur"""configuration File to be used to find weak passwords, if not specified, running config is taken as input.
		"""
		try :
			return self._config
		except Exception as e:
	...
	def configchanged(self) :
		ur"""returns True if configuration has changed since last saved config.
		"""
		try :
