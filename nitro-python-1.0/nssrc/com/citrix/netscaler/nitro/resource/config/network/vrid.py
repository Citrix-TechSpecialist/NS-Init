...
class vrid(base_resource) :
	...
	def __init__(self) :
		...
		self._tracking = ""
		self._ownernode = 0
		self._trackifnumpriority = 0
		self._preemptiondelaytimer = 0
		self._all = False
		self._ifaces = ""
		self._type = ""
	...
	@property
	def priority(self) :
		ur"""Base priority (BP), in an active-active mode configuration, which ordinarily determines the master VIP address.<br/>Default value: 255<br/>Maximum length =  255.
		"""
		try :
	...
	def trackifnumpriority(self) :
		ur"""Priority by which the Effective priority will be reduced if any of the tracked interfaces goes down.<br/>Default value: 0<br/>Minimum length =  1<br/>Maximum length =  255.
		"""
		try :
	...
	def preemptiondelaytimer(self) :
		ur"""Preemption delay time in seconds. If any high priority node will come in network, it will wait for these many seconds before becoming master.<br/>Default value: 0<br/>Minimum length =  1<br/>Maximum length =  36000.
		"""
		try :
