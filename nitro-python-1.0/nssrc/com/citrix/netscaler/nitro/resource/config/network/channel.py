...
class channel(base_resource) :
	...
	@property
	def id(self) :
		ur"""ID for the LA channel or cluster LA channel or LR channel to be created. Specify an LA channel in LA/x notation, where x can range from 1 to 8 or cluster LA channel in CLA/x notation or Link redundant channel in LR/x notation, where x can range from 1 to 4. Cannot be changed after the LA channel is created.
		"""
		try :
			return self._id
	...
	@property
	def lamac(self) :
		ur"""Specifies a MAC address for the LA channels configured in NetScaler virtual appliances (VPX). This MAC address is persistent after each reboot. 
	...
	@property
	def devicename(self) :
		ur"""LA channel name in form LA/x, where x is channel ID, which ranges from 1 to 8 or LR channel name in form LR/x, where x is channel ID, which ranges from 1 to 4.
		"""
		try :
	...
	@property
	def reqspeed(self) :
		ur"""Requested speed setting for this channel. Since no media are associated with LA, this speed is used to determine the threshold for the slave interfaces. If the speed of the member interface is less than the requested speed, that interface is considered inactive.<br/>Possible values = AUTO, 10, 100, 1000, 10000, 40000.
		"""
		try :
	...
	@property
	def actspeed(self) :
		ur"""Actual speed setting for this channel.<br/>Possible values = AUTO, 10, 100, 1000, 10000, 40000.
		"""
		try :
