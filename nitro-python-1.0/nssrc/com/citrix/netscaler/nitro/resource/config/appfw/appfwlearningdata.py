...
class appfwlearningdata(base_resource) :
	...
	def contenttype(self) :
		ur"""Content Type Name.<br/>Minimum length =  1.
		"""
		try :
	...
	def as_value_type_xss(self) :
		ur"""XSS value type. (Tag | Attribute | Pattern).<br/>Possible values = Tag, Attribute, Pattern.
		"""
		try :
	...
	def as_value_expr_xss(self) :
		ur"""XSS value expressions consistituting expressions for Tag, Attribute or Pattern.
		"""
		try :
	...
	def as_value_type_sql(self) :
		ur"""SQL value type. Keyword, SpecialString or Wildchar.<br/>Possible values = Keyword, SpecialString, Wildchar.
		"""
		try :
	...
	def as_value_expr_sql(self) :
		ur"""SQL value expressions consistituting expressions for Keyword, SpecialString or Wildchar.
		"""
		try :
	...
	def creditcardnumber(self) :
		ur"""The object expression that is to be excluded from safe commerce check.<br/>Minimum length =  1.
		"""
		try :
	...
	def creditcardnumberurl(self) :
		ur"""The url for which the list of credit card numbers are needed to be bypassed from inspection.<br/>Minimum length =  1.
		"""
		try :
	...
	def securitycheck(self) :
		ur"""Name of the security check.<br/>Possible values = startURL, cookieConsistency, fieldConsistency, crossSiteScripting, SQLInjection, fieldFormat, CSRFtag, XMLDoSCheck, XMLWSICheck, XMLAttachmentCheck, TotalXMLRequests, creditCardNumber, ContentType.
		"""
		try :
	...
	def fieldformatcharmappcre(self) :
		ur"""Form field value allowed character map.
		"""
		try :
