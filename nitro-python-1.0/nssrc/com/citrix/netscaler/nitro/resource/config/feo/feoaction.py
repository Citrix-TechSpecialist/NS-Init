...
class feoaction(base_resource) :
	...
	def __init__(self) :
		...
		self._imgtowebp = False
		self._imgtojpegxr = False
	...
	@property
	def imgtowebp(self) :
		ur"""Convert JPEG, GIF, PNG image formats to WEBP format.
		"""
		try :
	...
	def imgtojpegxr(self) :
		ur"""Convert JPEG, GIF, PNG image formats to JXR format.
		"""
		try :
	...
	def clientsidemeasurements(self) :
		ur"""Send AppFlow records about the web pages optimized by this action. The records provide FEO statistics, such as the number of HTTP requests that have been reduced for this page. You must enable the Appflow feature before enabling this parameter.
		"""
		try :
	...
	@property
	def builtin(self) :
		ur"""Flag to determine if front end optimization action is built-in or not.<br/>Possible values = MODIFIABLE, DELETABLE, IMMUTABLE, PARTITION_ALL.
		"""
		try :
