...
class auditnslogaction(base_resource) :
	...
	def serverdomainname(self) :
		ur"""Auditserver name as a FQDN. Mutually exclusive with serverIP.<br/>Minimum length =  1<br/>Maximum length =  255.
		"""
		try :
	...
	def domainresolveretry(self) :
		ur"""Time, in seconds, for which the NetScaler appliance waits before sending another DNS query to resolve the host name of the audit server if the last query failed.<br/>Default value: 5<br/>Minimum length =  5<br/>Maximum length =  20939.
		"""
		try :
	...
	@property
	def ip(self) :
		ur"""The resolved IP address of the auditserver.
		"""
		try :
			return self._ip
		except Exception as e:
