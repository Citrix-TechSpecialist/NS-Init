...
class nsacl(base_resource) :
	...
	@property
	def aclname(self) :
		ur"""Name for the extended ACL rule. Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters. Cannot be changed after the extended ACL rule is created.<br/>Minimum length =  1.
		"""
		try :
