...
class crvserver_stats(base_resource) :
	ur""" Statistics for CR virtual server resource.
	"""
	def __init__(self) :
		self._name = ""
		self._clearstats = ""
	...
	@property
	def name(self) :
		ur"""Name of a specific cache redirection virtual server.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		ur"""Name of a specific cache redirection virtual server.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	def  get(cls, service, name="", option_="") :
		ur""" Use this API to fetch the statistics of all crvserver_stats resources that are configured on netscaler.
		"""
		try :
			obj = crvserver_stats()
			if not name :
