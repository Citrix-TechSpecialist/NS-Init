...
class gslbvserver_stats(base_resource) :
	ur""" Statistics for Global Server Load Balancing Virtual Server resource.
	"""
	def __init__(self) :
		self._name = ""
		self._clearstats = ""
	...
	@property
	def name(self) :
		ur"""Name of the GSLB virtual server for which to display statistics. If you do not specify a name, statistics are displayed for all GSLB virtual servers.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		ur"""Name of the GSLB virtual server for which to display statistics. If you do not specify a name, statistics are displayed for all GSLB virtual servers.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	def totvserverdownbackuphits(self) :
		ur"""Number of times traffic was diverted to backup vserver since primary vserver was DOWN.
		"""
		try :
	...
	@property
	def inactsvcs(self) :
		ur"""number of INACTIVE services bound to a vserver.
		"""
		try :
	...
	def  get(cls, service, name="", option_="") :
		ur""" Use this API to fetch the statistics of all gslbvserver_stats resources that are configured on netscaler.
		"""
		try :
