...
class authenticationloginschemapolicy_stats(base_resource) :
	...
	@property
	def name(self) :
		ur"""The name of the LoginSchema policy for which statistics will be displayed.  If not given statistics are shown for all policies.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		ur"""The name of the LoginSchema policy for which statistics will be displayed.  If not given statistics are shown for all policies.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	def  get(cls, service, name="", option_="") :
		ur""" Use this API to fetch the statistics of all authenticationloginschemapolicy_stats resources that are configured on netscaler.
		"""
		try :
