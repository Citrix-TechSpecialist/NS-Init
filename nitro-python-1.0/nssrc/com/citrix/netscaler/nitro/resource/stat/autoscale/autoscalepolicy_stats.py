...
class autoscalepolicy_stats(base_resource) :
	ur""" Statistics for Autoscale policy resource.
	"""
	def __init__(self) :
		self._name = ""
		self._clearstats = ""
	...
	@property
	def name(self) :
		ur"""The name of the autoscale policy for which statistics will be displayed.  If not given statistics are shown for all autoscale policies.<br/>Minimum length =  1.
		"""
		try :
			return self._name
	...
	@name.setter
	def name(self, name) :
		ur"""The name of the autoscale policy for which statistics will be displayed.  If not given statistics are shown for all autoscale policies.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	def  get(cls, service, name="", option_="") :
		ur""" Use this API to fetch the statistics of all autoscalepolicy_stats resources that are configured on netscaler.
		"""
		try :
