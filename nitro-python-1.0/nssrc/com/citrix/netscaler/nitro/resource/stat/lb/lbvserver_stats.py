...
class lbvserver_stats(base_resource) :
	ur""" Statistics for Load Balancing Virtual Server resource.
	"""
	def __init__(self) :
		self._name = ""
		self._clearstats = ""
	...
	@name.setter
	def name(self, name) :
		ur"""Name of the virtual server. If no name is provided, statistical data of all configured virtual servers is displayed.
		"""
		try :
			self._name = name
		except Exception as e:
	...
	def cursubflowconn(self) :
		ur"""Current Multipath TCP subflows.
		"""
		try :
	...
	def vsvrsurgecount(self) :
		ur"""Number of requests waiting on this vserver. .
		"""
		try :
	...
	def curmptcpsessions(self) :
		ur"""Current Multipath TCP sessions.
		"""
		try :
	...
	def svcsurgecount(self) :
		ur"""Total number of requests in the surge queues of all the services bound to this LB-vserver.
		"""
		try :
	...
	def  get(cls, service, name="", option_="") :
		ur""" Use this API to fetch the statistics of all lbvserver_stats resources that are configured on netscaler.
		"""
		try :
			obj = lbvserver_stats()
			if not name :
