...
class audit_stats(base_resource) :
	...
	def auditsyslogmsgsenttcprate(self) :
		ur"""Rate (/s) counter for auditsyslogmsgsenttcp.
		"""
		try :
	...
	def logsdroppedtxminnsbs(self) :
		ur"""Total number of log messages dropped by NetScaler when NSBQ length is less than TX min NSBs.
		"""
		try :
	...
	def logsdroppedrate(self) :
		ur"""Rate (/s) counter for logsdropped.
		"""
		try :
	...
	def logunsentlbsys(self) :
		ur"""Total auditlog messages which are not delivered to load balanced syslog servers.
		"""
		try :
	...
	def systcpconnfailrate(self) :
		ur"""Rate (/s) counter for systcpconnfail.
		"""
		try :
	...
	def systcpconnfail(self) :
		ur"""Failures in establishment of a connection between the NetScaler and the syslog server.
		"""
		try :
	...
	def logunsentlbsysrate(self) :
		ur"""Rate (/s) counter for logunsentlbsys.
		"""
		try :
	...
	def logsdropped(self) :
		ur"""Total number of log messages dropped by NetScaler after max hold limit is reached.
		"""
		try :
	...
	def auditsyslogmsgsent(self) :
		ur"""Syslog messages sent to the syslog server(s) over UDP.
		"""
		try :
	...
	def logsdroppedtxminnsbsrate(self) :
		ur"""Rate (/s) counter for logsdroppedtxminnsbs.
		"""
		try :
	...
	def auditsyslogmsgsenttcp(self) :
		ur"""Syslog messages sent to the syslog server(s) over TCP.
		"""
		try :
